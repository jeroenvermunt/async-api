...
from pysdk import ApiSDK, return_types


class Cats(ApiSDK):
    endpoints = {"image": "https://http.cat/{status_code}"}

    return_type = return_types.IMAGE


class Dogs(ApiSDK):
    endpoints = {"image": "https://http.dog/{status_code}.jpg"}
...
async def cat_and_dog(status_code):
    ...
    async with asyncio.TaskGroup() as tg:
        cat_image = tg.create_task(cats.image(status_code))

        dog_image = tg.create_task(dogs.image(status_code))
...
async def three_cats():
    ...
    async with cats:
        async with asyncio.TaskGroup() as tg:
            cat_one = tg.create_task(cats.image(code_1 := random_status_code()))

            cat_two = tg.create_task(cats.image(code_2 := random_status_code()))

            cat_three = tg.create_task(cats.image(code_3 := random_status_code()))
